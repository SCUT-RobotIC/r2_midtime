...
#define m3519_2_1_brake m3519_init-90*8191/360
#define m3519_2_2_brake m3519_init-30*8191/360
#define m3519_2_3_brake	m3519_init+30*8191/360
...
void ctrlmotor(double Vx, double Vy, double omega,int brake) {
	...
	if(brake==1)
	{
		...
		if((int)MotorSignal[0].thetas%360>180)
			set_target(2,1,9*((-((int)(MotorSignal[0].thetas)/360)*360)*8191/(360)-8191+m3519_2_1_brake)+HelmInit[0]);
		else
			set_target(2,1,9*((-((int)(MotorSignal[0].thetas)/360)*360)*8191/(360)+m3519_2_1_brake)+HelmInit[0]);
		
		if((int)MotorSignal[1].thetas%360>180)
			set_target(2,2,9*((-((int)(MotorSignal[1].thetas)/360)*360)*8191/(360)-8191+m3519_2_2_brake)+HelmInit[1]);
		else
			set_target(2,2,9*((-((int)(MotorSignal[1].thetas)/360)*360)*8191/(360)+m3519_2_2_brake)+HelmInit[1]);
		
		if((int)MotorSignal[2].thetas%360>180)
			set_target(2,3,9*((-((int)(MotorSignal[2].thetas)/360)*360)*8191/(360)-8191+m3519_2_3_brake)+HelmInit[2]);
		else
			set_target(2,3,9*((-((int)(MotorSignal[2].thetas)/360)*360)*8191/(360)+m3519_2_3_brake)+HelmInit[2]);
		return;
	}
  ...
  MotorSignal[2].thetan = atan2(Vy + omega * cos(30.0 * PI / 180.0), Vx + omega * sin(30.0 * PI / 180.0)) * 180 / PI;
	...
	set_target(2,1,9*-MotorSignal[0].thetas*8191/(360)+9*m3519_init+HelmInit[0]);
	set_target(2,2,9*-MotorSignal[1].thetas*8191/(360)+9*m3519_init+HelmInit[1]);
	set_target(2,3,9*-MotorSignal[2].thetas*8191/(360)+9*m3519_init+HelmInit[2]);
	 ...
	 if((fabs(motor_data_can2[0]->ecd+motor_data_can2[0]->circle*8191-rtU.target_CH2_1)<45*8191/360&&
		 fabs(motor_data_can2[1]->ecd+motor_data_can2[1]->circle*8191-rtU.target_CH2_2)<45*8191/360&&
		 fabs(motor_data_can2[2]->ecd+motor_data_can2[2]->circle*8191-rtU.target_CH2_3)<45*8191/360)
		 ||omega!=0){
				if(MotorSignal[0].dir==1)
					motor[Motor1].ctrl.vel_set=-sqrt(pow(Vx-omega,2)+pow(Vy,2))*mult*20/9000;
				else
					motor[Motor1].ctrl.vel_set=sqrt(pow(Vx-omega,2)+pow(Vy,2))*mult*20/9000;
		
				if(MotorSignal[1].dir==1)	
					motor[Motor2].ctrl.vel_set=-sqrt(pow((Vx+omega*sin(30.0*PI/180.0)),2)+pow((Vy-omega*cos(30.0*PI/180.0)),2))*mult*20/9000;
				else
					motor[Motor2].ctrl.vel_set=sqrt(pow((Vx+omega*sin(30.0*PI/180.0)),2)+pow((Vy-omega*cos(30.0*PI/180.0)),2))*mult*20/9000;
		
				if(MotorSignal[2].dir==1)
					motor[Motor3].ctrl.vel_set=-sqrt(pow((Vx+omega*sin(30.0*PI/180.0)),2)+pow((Vy+omega*cos(30.0*PI/180.0)),2))*mult*20/9000;	
				else
					motor[Motor3].ctrl.vel_set=sqrt(pow((Vx+omega*sin(30.0*PI/180.0)),2)+pow((Vy+omega*cos(30.0*PI/180.0)),2))*mult*20/9000;	
...
void set_reset_status(){	
	...
	if(//(fabs(rtU.target_CH2_1-motor_data_can2[0]->speed_rpm)<20&&rtU.status_CH2_1==1)||
		(fabs(rtU.target_CH2_1-motor_data_can2[0]->circle*8191-motor_data_can2[0]->ecd)<3&&rtU.status_CH2_1==2))
		rtU.reset_status2_1=1-rtU.reset_status2_1;
	
	if(//(fabs(rtU.target_CH2_2-motor_data_can2[1]->speed_rpm)<20&&rtU.status_CH2_2==1)||
		(fabs(rtU.target_CH2_2-motor_data_can2[1]->circle*8191-motor_data_can2[1]->ecd)<3&&rtU.status_CH2_2==2))
		rtU.reset_status2_2=1-rtU.reset_status2_2;
	
	if(//(fabs(rtU.target_CH2_3-motor_data_can2[2]->speed_rpm)<20&&rtU.status_CH2_3==1)||
		(fabs(rtU.target_CH2_3-motor_data_can2[2]->circle*8191-motor_data_can2[2]->ecd)<3&&rtU.status_CH2_3==2))
		rtU.reset_status2_3=1-rtU.reset_status2_3;
	
	if(//(fabs(rtU.target_CH2_4-motor_data_can2[3]->speed_rpm)<20&&rtU.status_CH2_4==1)||
		(fabs(rtU.target_CH2_4-motor_data_can2[3]->circle*8191-motor_data_can2[3]->ecd)<3&&rtU.status_CH2_4==2))
